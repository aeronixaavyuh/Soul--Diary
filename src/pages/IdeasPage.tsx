...
import {
  Plus, Lightbulb, Heart, Trash2,
  Wand2, Loader2, ChevronLeft, X,
  Check, RefreshCw, Search, Sparkles,
  Zap, ArrowRight, LayoutGrid, List,
  Star, BookOpen,
} from 'lucide-react'
...
export default function IdeasPage() {
  ...
  const handleExpandIdea = useCallback(async () => {
    ...
    try {
      const res = await window.electronAPI.ai.expandIdea(text)
      if (res?.success && res.data) {
        setExpandedIdea(res.data)
      } else {
        setExpandedIdea({
          expanded: 'This idea has great potential. Consider exploring it further.',
          keyPoints: [
            'Define the core problem this idea solves',
            'Identify your target audience',
            'Consider the resources needed',
          ],
          nextSteps: [
            'Do initial research',
            'Create a simple prototype or outline',
      ...
      setExpandedIdea({
        expanded: 'Develop this idea by breaking it into actionable steps.',
        keyPoints: ['Core concept', 'Target audience', 'Unique value'],
        nextSteps: ['Research', 'Plan', 'Execute'],
      })
    } finally {
      setAiLoading(false)
    }
  ...
  // ── Apply expanded idea to editor ─────────────────────────────────────────
  const handleApplyExpansion = useCallback(() => {
    if (!expandedIdea || !editorRef.current) return
      ...
      content: [
        {
          type: 'heading',
          attrs: { level: 2 },
          content: [{ type: 'text', text: '💡 Idea Overview' }],
        },
        {
          type: 'paragraph',
          content: [{ type: 'text', text: expandedIdea.expanded }],
        },
        {
          type: 'heading',
          attrs: { level: 3 },
        ...
        {
          type: 'bulletList',
          content: expandedIdea.keyPoints.map(p => ({
            type: 'listItem',
        ...
        {
          type: 'orderedList',
          content: expandedIdea.nextSteps.map(s => ({
            type: 'listItem',
    ...
    handleEditorChange(content, expandedIdea.expanded)
    setShowAiPanel(false)
  }, [expandedIdea, handleEditorChange])
  ...
  const handleInsertImage = useCallback(() => {
    ...
    input.onchange = (e) => {
      ...
      reader.onload = () => editorRef.current?.insertImage(reader.result as string, file.name)
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      border: `1px solid ${isSelected ? idea.color + '50' : 'var(--border)'}`,
                      transition: 'all 0.15s',
                      position: 'relative',
                      overflow: 'hidden',
                    }}
                    onMouseEnter={e => {
                      if (!isSelected) {
                        (e.currentTarget as HTMLElement).style.background = theme.bg
                          ; (e.currentTarget as HTMLElement).style.borderColor = idea.color + '40'
                      }
                    }}
                    onMouseLeave={e => {
                      if (!isSelected) {
              ...
              return (
                ...
                >
                  ...
                  <div style={{
                    ...
                    {idea.is_favorite ? <Star size={9} style={{ color: '#fbbf24', fill: '#fbbf24' }} /> : null}
                    {formatTime(idea.updated_at)}
                  </div>
                </div>
              )
                ...
                <button
                  ...
                  style={{
                    ...
                    background: showAiPanel
                      ? 'rgba(251,191,36,0.1)' : 'var(--bg-tertiary)',
                    color: showAiPanel ? '#d97706' : 'var(--text-secondary)',
                    borderColor: showAiPanel ? 'rgba(251,191,36,0.35)' : 'var(--border)',
                    fontSize: '0.75rem',
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                      ...
                      <div style={{ flex: 1, minWidth: '160px' }}>
                        ...
                        {expandedIdea.keyPoints.map((p, i) => (
                          <div key={i} style={{
                            display: 'flex',
                      ...
                      <div style={{ flex: 1, minWidth: '160px' }}>
                        ...
                        {expandedIdea.nextSteps.map((s, i) => (
                          <div key={i} style={{
                            display: 'flex',
                            alignItems: 'flex-start',
                            gap: '0.4rem',
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '0.875rem', maxWidth: '280px', lineHeight: 1.6 }}>
                {allIdeas.length === 0
                  ? 'Got a thought? Capture it before it slips away. Use Quick Capture or create a full idea.'
