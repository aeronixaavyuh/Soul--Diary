...
const ollama = {
  ...
  setModel: (_model) => void 0,
  getModel: () => null,
  pullModel: async (_model, _onProgress) => ({ success: false, error: "Ollama unavailable" }),
  chat: async (_prompt, _context) => ({ success: false, error: "Ollama unavailable" }),
  detectMood: async (_text) => ({
    success: true,
    data: {
      mood: "neutral",
      score: 5,
  ...
  summarize: async (_text, _style = "brief") => ({ success: false, error: "Ollama unavailable" }),
  getSuggestions: async (_text, _type = "continuation") => ({ success: false, error: "Ollama unavailable" }),
  getDailyPrompts: async (_context) => ({ success: false, error: "Ollama unavailable" }),
  getPredictions: async (_prefix, _context = "any", _limit = 8) => []
};
function registerAiHandlers() {
  electron.ipcMain.handle("ai:isAvailable", async () => {
    try {
  ...
  electron.ipcMain.handle("ai:models", async () => {
    try {
  ...
  electron.ipcMain.handle("ai:setModel", (_e, modelName) => {
    try {
  ...
  electron.ipcMain.handle("ai:getModel", () => {
    return { success: true, model: ollama.getModel() };
  });
  electron.ipcMain.handle("ai:pullModel", async (event, modelName) => {
    try {
  ...
  electron.ipcMain.handle("ai:query", async (_e, prompt, context) => {
    try {
      if (!prompt?.trim()) {
  ...
  electron.ipcMain.handle("ai:detectMood", async (_e, text) => {
    try {
      if (!text?.trim()) {
        return {
          success: true,
  ...
  electron.ipcMain.handle("ai:summarize", async (_e, text, style = "brief") => {
    try {
      if (!text?.trim()) {
  ...
  electron.ipcMain.handle("ai:suggestions", async (_e, text, type = "continuation") => {
    try {
      if (!text?.trim()) {
  ...
  electron.ipcMain.handle("ai:dailyPrompts", async (_e, context) => {
    try {
  ...
  electron.ipcMain.handle("ai:predictWords", async (_e, text, count = 3) => {
    try {
  ...
  electron.ipcMain.handle("ai:expandIdea", async (_e, idea) => {
    try {
      if (!idea?.trim()) {
  ...
  electron.ipcMain.handle("ai:autoTitle", async (_e, text) => {
    try {
  ...
  electron.ipcMain.handle("ai:batchMood", async (_e, entries) => {
    try {
      const results = [];
  ...
  electron.ipcMain.handle("ai:analyzeStyle", async (_e, text) => {
    try {
...
clarity is 1-10. wordCount and avgSentenceLength are numbers.`;
  ...
  electron.ipcMain.handle("ai:extractGratitude", async (_e, text) => {
    try {
